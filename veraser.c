...
/* Encrypt buffer in place with AES-256-CTR using Windows CNG */
static int ve_aes_ctr_encrypt_windows(unsigned char* buf, size_t len, const unsigned char key[32], unsigned char iv[16]) {
    ...
    PUCHAR keyObj = NULL;
    DWORD keyObjLen = 0, blockLen = 0, resLen = 0;
    DWORD cb = 0;
    ...
    if (BCryptSetProperty(hAlg, BCRYPT_CHAINING_MODE, (PUCHAR)BCRYPT_CHAIN_MODE_CTR, (ULONG)sizeof(BCRYPT_CHAIN_MODE_CTR), 0) != 0) { 
        ve_set_last_errorf("SetProperty CTR failed"); 
        goto fail; 
    }
    if (BCryptGetProperty(hAlg, BCRYPT_OBJECT_LENGTH, (PUCHAR)&keyObjLen, sizeof(keyObjLen), &cb, 0) != 0) { 
        ve_set_last_errorf("GetProperty OBJ_LEN failed"); 
        goto fail; 
    }
    if (BCryptGetProperty(hAlg, BCRYPT_BLOCK_LENGTH, (PUCHAR)&blockLen, sizeof(blockLen), &cb, 0) != 0) { 
        ve_set_last_errorf("GetProperty BLK_LEN failed"); 
        goto fail; 
    }
    keyObj = (PUCHAR)malloc(keyObjLen);
    if (!keyObj) { 
        ve_set_last_errorf("malloc keyObj failed"); 
        goto fail; 
    }
    if (BCryptGenerateSymmetricKey(hAlg, &hKey, keyObj, keyObjLen, (PUCHAR)key, 32, 0) != 0) { 
    ...
    while (offset < len) {
        ...
        if (chunk > 1<<20) chunk = 1<<20; /* call in modest chunks */
        DWORD out = 0;
        unsigned char iv_local[16];
        memcpy(iv_local, iv, 16);
        if (BCryptEncrypt(hKey, buf + offset, (ULONG)chunk, NULL, iv_local, 16, buf + offset, (ULONG)chunk, &out, 0) != 0 || out != chunk) {
            ve_set_last_errorf("BCryptEncrypt failed"); goto fail;
        }
        uint64_t blocks = (chunk + blockLen - 1) / blockLen;
    ...
    ve_secure_bzero(keyObj, keyObjLen); free(keyObj);
    return 0;
fail:
    if (hKey) BCryptDestroyKey(hKey);
    if (hAlg) BCryptCloseAlgorithmProvider(hAlg, 0);
...
#ifdef VE_USE_OPENSSL
/* Encrypt buffer in place with AES-256-CTR using OpenSSL */
static int ve_aes_ctr_encrypt_openssl(unsigned char* buf, size_t len, const unsigned char key[32], const unsigned char iv[16]) {
    ...
    int outl = 0;
    if (EVP_EncryptUpdate(ctx, buf, &outl, buf, (int)len) != 1 || outl != (int)len) { 
...
static int ve_write_pattern_fd(int fd, uint64_t file_size, unsigned char pattern) {
    const size_t chunk = VE_DEFAULT_CHUNK_SIZE;
    ...
    while (written < file_size) {
        size_t to_write = (size_t)((file_size - written) < chunk ? (file_size - written) : chunk);
#if defined(_WIN32)
        DWORD w = 0;
        if (!WriteFile((HANDLE)_get_osfhandle(fd), buf, (DWORD)to_write, &w, NULL)) {
...
static int ve_encrypt_file_in_place_aesctr(int fd, uint64_t file_size) {
    ...
    unsigned char iv[16];
    if (ve_csrand(key, sizeof(key)) != 0 || ve_csrand(iv, sizeof(iv)) != 0) { 
        free(buf); 
        return -1; 
    }
    ...
    while (processed < file_size) {
        size_t to_io = (size_t)((file_size - processed) < chunk ? (file_size - processed) : chunk);
#if defined(_WIN32)
        DWORD r = 0;
        if (!ReadFile((HANDLE)_get_osfhandle(fd), buf, (DWORD)to_io, &r, NULL) || r == 0) { 
        ...
        size_t readn = (size_t)r;
        if (ve_aes_ctr_encrypt_windows(buf, readn, key, iv) != 0) {
             free(buf); 
             return -1; 
        }
        ...
        DWORD w = 0;
        if (!WriteFile((HANDLE)_get_osfhandle(fd), buf, (DWORD)readn, &w, NULL) || w != readn) { 
            ve_set_last_errorf("WriteFile failed"); 
            free(buf); 
            return -1; 
        }
        ...
        size_t readn = (size_t)r;
        if (ve_aes_ctr_encrypt_openssl(buf, readn, key, iv) != 0) {
            free(buf); 
            return -1; 
        }
...
#else
        /* Fallback: build without OpenSSL on POSIX. Strongly recommend enabling VE_USE_OPENSSL. */
        ssize_t r = read(fd, buf, to_io);
        if (r <= 0) { 
            ve_set_last_errorf("read failed: %s", strerror(errno)); 
            free(buf); 
    ...
    ve_flush_fd(fd);
    ve_secure_bzero(key, sizeof(key));
    ve_secure_bzero(iv, sizeof(iv));
    ve_secure_bzero(buf, chunk);
    free(buf);
    return 0;
}
...
/* Walk a directory recursively and erase files; remove dirs when empty */
static ve_status_t ve_walk_and_erase(const char* path, const ve_options_t* opt) {
    ...
    if (h == INVALID_HANDLE_VALUE) {
        /* empty or inaccessible; attempt to remove dir */
    ...
    do {
        ...
        if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ve_walk_and_erase(child, opt);
            ve_remove_empty_dir(child);
        } else {
            ve_erase_single_file(child, opt);
        }
...
static ve_status_t ve_erase_single_file(const char* path, const ve_options_t* opt) {
    if (opt && opt->dry_run) return VE_SUCCESS;
